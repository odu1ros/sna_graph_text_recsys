...
class LightGCNEncoder(nn.Module):
    ...
    def forward(self, edge_index):
        ...
        for _ in range(self.num_layers):
            ...
            msg_ui = i_emb[dst] * norm.view(-1, 1)
            msg_iu = u_emb[src] * norm.view(-1, 1)
...
def main(args):
    ...
    encoder = LightGCNEncoder(len(user_map), len(biz_map), args.out_dim, num_layers=args.num_layers).to(device)
